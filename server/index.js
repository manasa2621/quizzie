    ...
    const totalQuestions = quizzes.reduce((sum, quiz) => {
      return (
        ...
        quiz.questions.reduce((questionSum, questionSet) => {
          const pollQuestion = questionSet.pollQuestion;
          if (pollQuestion) {
            return questionSum + Object.keys(pollQuestion).length;
          } else {
            return questionSum; // Don't add anything if pollQuestion is null or undefined
          }
        }, 0)
      );
    }, 0);
